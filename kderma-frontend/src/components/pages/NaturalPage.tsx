...
const NaturalPage: React.FC = () => {
  ...
  const [filteredProducts, setFilteredProducts] = useState(mockProducts.filter(product =>
    product.category.toLowerCase().includes('natural')
  ));
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">
            Pure, natural ingredients for wholesome beauty
          </p>
        </div>
        ...
        <div className="mb-8">
          <div className="flex flex-wrap justify-center gap-4 mb-6">
            <Button
              variant={selectedCategory === 'Natural' ? "primary" : "outline"}
              onClick={() => {
            ...
            {NAVIGATION_ITEMS.categories.filter(cat => cat !== 'Natural').map(category => (
              <Button
                key={category}
